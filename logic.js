class TableroDeJuego {
  ...
  #numeroDeIntentos = 0;
  #paresEncontrados = 0;
  #juegoActivo = false; // Por defecto, está inactivo
  ...
  generarTablero() {
    ...
    this.#cargarIconos();
    this.#selecciones = [];
    this.#juegoActivo = false; // Bloquea el juego al inicio
    ...
    // Activa el juego tras 5 segundos (duración de la animación)
    setTimeout(() => {
      this.#juegoActivo = true; // Ahora se puede jugar
    }, 5000);
  }

  seleccionarTarjeta(i) {
    if (!this.#juegoActivo) {
      return; // Evita la selección si el juego no ha comenzado
    }
  ...
  #deseleccionar(selecciones) {
    setTimeout(() => {
      let caraPosterior1 = document.getElementById(
        "cara-posterior" + selecciones[0]
      );
      let caraPosterior2 = document.getElementById(
        "cara-posterior" + selecciones[1]
      );
